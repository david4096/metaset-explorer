...
# add better sentence vectors
mapper = umap.UMAP(metric='cosine', low_memory=True, n_components=6)
umapped1 = mapper.fit_transform(imputed)

vec2 = []

for ds, v in zip(umapped1, cleandss):
    vec2.append(list(ds) + list(model.encode(v['description'], precision='binary')))


mapper2 = umap.UMAP(metric='cosine', low_memory=True, n_components=3)
umapped = mapper.fit_transform(vec2)
...
df.to_parquet('hfdatasets6_newsentence.parquet')
